...
/* Return a single item from Ring Buffer without pop */
int RingBuffer_Get(RINGBUFF_T *RingBuff, int index, void *data)
{
	uint8_t *ptr = RingBuff->data;
	...
	if (RingBuffer_IsEmpty(RingBuff))
		return 0;
	/* We cannot pop when index is out of range */
	if (index >= RingBuffer_GetCount(RingBuff) || index < 0)
		return 0;

	ptr += ((RingBuff->head - index - 1) % RingBuff->count) * RingBuff->itemSz;
	memcpy(data, ptr, RingBuff->itemSz);
